...
Paths (relative to repo root)
    ...
    OUTPUT FILE: artifacts/system_assessment_chastain2.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.continuous_intelligence_chastain
...
DATA_FILE: Final[Path] = DATA_DIR / "new_data.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "system_assessment_chastain2.csv"
...
def main() -> None:
    ...
    df = df.with_columns(
        [
            ...
            (pl.col("latency_ms") / pl.col("requests")).alias("avg_latency_ms"),
        ]
    )
